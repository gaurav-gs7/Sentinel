...
Attesta is a Kubernetes reliability platform with a Harbor-style onboarding path and a Helios-style workflow execution substrate. Attesta owns reliability policy and Kubernetes operations; workflow execution owns ordered steps, retries, state transitions, and audit events.
...
Attesta does not run a second standalone control plane next to Helios. Instead, Attesta embeds Helios-style orchestration where reliability operations need durable execution semantics:
